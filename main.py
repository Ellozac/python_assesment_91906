"""Order preparation management software."""
        ...
        with open(self.file_path, 'r') as file:
            try:
                ...
                tk.messagebox.showerror(
                    title="Error Invalid Json", message=f"Invalid JSON. {self.file_path}")
...
class App:
    ...
    def check_robot(self):
        ...
        if not int(ans) == 19:
            messagebox.showerror(
                "Wrong Answer", "AI DETECTED!!!!. program closing.")
            exit(1)
    ...
    def main(self):
        ...
        if self.orders.remaining_orders():
            ...
            self.clear_window()
            tk.Label(
                self.root, text=f"Preparing order {order.order_num}").pack()
            # Display the items in the order
            for i in order.items:
                tk.Label(
                    self.root, text=f"{i['name']} x {i['quantity']}").pack()

            tk.Label(
                self.root, text=f"Total price: ${order.total_price}").pack()

            tk.Button(self.root, text="Complete",
                      command=self.main).pack(pady=5)
            tk.Button(self.root, text='Skip',
                      command=lambda: self.skip_order(order)).pack(pady=5)
        else:
    ...
    def check_for_orders(self):
        ...
        if self.orders.load_orders():
            ...
            self.clear_window()
            no_order_label = tk.Label(
                self.root, text="There are currently no orders.")
            no_order_label.pack()
